...
class Graph {
	...
	private showTooltip(id: number, vertexScreenY: number) {
		...
		let html = '<div class="graphTooltipTitle">提交节点：' + abbrevCommit(this.commits[id].hash) + '</div>';
		if (this.commitHead !== null && typeof this.commitLookup[this.commitHead] === 'number') {
			html += '<div class="graphTooltipSection">此提交' + (childrenIncludesHead ? '' : '<b><i>没有</i></b> ') + '包括在 <span class="graphTooltipRef">HEAD</span> 中</div>';
		}
		...
		if (stashes.length > 0) {
			...
			html += '<div class="graphTooltipSection">暂存区：' + getLimitedRefs(htmlRefs) + '</div>';
		}
