...
class FindWidget {
	...
	constructor(view: GitGraphView) {
		...
		this.widgetElem.innerHTML = '<input id="findInput" type="text" placeholder="查找" disabled/><span id="findCaseSensitive" class="findModifier" title="区分大小写">Aa</span><span id="findRegex" class="findModifier" title="使用正则表达式">.*</span><span id="findPosition"></span><span id="findPrev" title="上一个匹配项 (Shift+Enter)"></span><span id="findNext" title="下一个匹配项 (Enter)"></span><span id="findOpenCdv" title="在打开的提交细节视图中查找"></span><span id="findClose" title="关闭 (Escape)"></span>';
	...
	private findMatches(goToCommitHash: string | null, scrollToCommit: boolean) {
		...
		if (this.text !== '') {
			...
			if (findPattern !== null && findGlobalPattern !== null) {
				...
				if (zeroLengthMatch) {
					this.widgetElem.setAttribute(ATTR_ERROR, '不能使用长度匹配为零的正则表达式');
					this.clearMatches();
					this.matches = [];
				}
			}
	...
	private updatePosition(position: number, scrollToCommit: boolean) {
		...
		this.positionElem.innerHTML = this.matches.length > 0 ? (this.position + 1) + ' of ' + this.matches.length : '无结果';
		this.view.saveState();
	}
